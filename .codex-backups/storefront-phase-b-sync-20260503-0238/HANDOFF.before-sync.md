...
- Telegram-уведомления о заказах настроены на production: `TELEGRAM_BOT_TOKEN` и `TELEGRAM_MANAGER_CHAT_ID` заполнены в `/var/www/climat-simf.ru/.env`, PM2 перезапущен с `--update-env`.
- Сделан тестовый заказ через production `createLocalOrder`: `ORD-20260502-MXK3DM`, уведомление ушло в Telegram-группу.
...
2. Решить вопрос с расширенными характеристиками:
...
Продолжи проект `БытТехОпт`: прочитай `web-store/HANDOFF.md`, проверь VPS `212.116.115.150`, проверь сайт `https://climat-simf.ru`, PM2 `climat-simf-store`, cron-логи в `/var/log/climat-simf.ru/`, затем проверь клиентский путь вручную и реши вопрос с расширенными характеристиками.
