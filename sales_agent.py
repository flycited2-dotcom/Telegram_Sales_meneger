...
_STOP = {
    ...
    "сделаете","сделать","можете","можно","могу","будете","буду","будет",
    "привезёте","привезете","доставите","доставка","привезти",
    # Вопросительные
    ...
    # Предлоги и союзы
    "про","для","из","от","до","за","при","под","над","без",
    "со","и","в","на","с","по","но","так","не","у","а","к","ко","об","обо",
    # Разговорные/бытовые (никогда не товар)
    "нет","да","ну","ок","окей","ладно","хорошо","понятно","именно",
    "точно","конечно","спасибо","пожалуйста","надо","всё","все","ничего","ничё",
    "привет","здравствуйте","добрый","день","утро","вечер","пока","доброе",
    "тут","здесь","там","ага","угу","ясно","давай","давайте","понял","поняла",
    "нормально","нормал","ладно","блин","эй","ой","ай","ух","хм","эм",
    "тормоз","тормозит","тормози","стоп","хватит","вообще","совсем",
    # Финансовые (не товар)
    "скидку","скидка","скидки","скидок","скидочку","акция","акции",
    "стоит","цена","цены","рублей","рубль","тысяч","тысячи","штук","шт",
    "дешево","дешевле","дорого","дороже","бюджет","бюджета","бесплатно",
    # Заказ/действие (не товар)
    "поставь","ставь","оформи","запиши","бери","возьму","возьмём",
    "скинь","выбери","заказ","заказа","заявку","заявка",
    # Указатели (вызывают ложные совпадения)
    "одно","одна","одну","один","оба","обе","несколько","пару","любой",
    "которая","который","которое","которые","такой","такая","такие","первый",
    "второй","третий","первую","второй","третью","последний","этот",
    # Сравнения
    "очень","более","менее","лучше","хуже","больше","меньше",
    "также","тоже","ещё","еще","просто","только","уже","самое",
    # Единицы измерения
...
_SYNONYMS = {
    ...
    # Холодильники
    "холодос":       "холодильник",
    "холодоса":      "холодильник",
    # Бойлеры (в каталоге — «Водонагреватель»)
    "бойлер":        "водонагреватель",
    "бойлеры":       "водонагреватель",
    "бойлера":       "водонагреватель",
    "бойлеров":      "водонагреватель",
...
_SYSTEM_PROMPT = """\
Ты — {name}, менеджер «{company}». Отвечаешь коротко, по-деловому, как живой человек. Никогда не раскрывай что ты бот.

ДАННЫЕ — ТОЛЬКО ИЗ [КАТАЛОГ]
Если в сообщении есть блок [КАТАЛОГ] с товарами — используй только их. Не придумывай модели и цены.
Если [КАТАЛОГ] пуст или помечен как «не задействован» — это не товарный запрос, просто общайся.
Если [КАТАЛОГ] говорит «товар не найден» — скажи что уточнишь у поставщика, предложи что-то похожее из контекста если есть.

ФОРМАТ ОТВЕТА
Без markdown, нумерации, тире в начале строк.
Товары: каждый на новой строке «Название — цена».
Показывай все найденные товары из [КАТАЛОГ] — не ограничивай список.
Один вопрос в конце, не больше.

ПРИМЕРЫ (показывают стиль ответов — цены и модели ТОЛЬКО из блока [КАТАЛОГ], не из примеров)
...
Клиент: тысяч 30
Ты: [берёшь все товары из блока [КАТАЛОГ] и пишешь каждый на новой строке: Название — цена]
Что ставим?

Клиент: бойлер есть на 80 литров?
Ты: Есть. [цена из КАТАЛОГ]. Брать?
...
Клиент: сделаете скидку на 5 штук?
Ты: Посмотрю что можно сделать

Клиент: такой модели нет?
Ты: Нет пока, уточню у поставщика

Клиент: ничего
Ты: Хорошо, буду на связи
...
Ты: Принял, Иван. Уточню у поставщика и напишу.\
"""
...
class SalesAgent:
    ...
    def _rag(self, text: str) -> str:
        ...
        if not tokens:
            # Smalltalk / greeting / frustration — no product query detected
            return "[КАТАЛОГ: не задействован]"
        ...
        if not scored:
            return "[КАТАЛОГ: товар не найден]"
