...
_STOP = {
    ...
    "дайте","покажи","покажите","расскажи","расскажите","купить","куплю",
    "заказать","хочется","подойдет","подойдёт","посоветуйте","посоветуй",
    "помогите","помоги","ищу","найдите","найди","интересует","интересуют",
    "скажи","скажите","дай","дайте","подбери","подберите","смотри","смотрите",
    # Вопросительные
    "есть","ли","что","как","где","сколько","какой","какая","какие","каких",
    "чего","кого","кому","зачем","почему","когда","куда","откуда",
    # Местоимения и артикли
    "это","этот","эта","эти","этого","той","тот","та","те","тех","того",
    "самый","самая","сам","сама","себя","свой","своя","свои","своих",
    "меня","мне","нас","нам","вас","вам","его","её","их","им","ими",
    # Предлоги
    "про","для","купить","из","от","до","за","при","под","над","без",
    "со","и","в","на","с","по","но","так","не","у","а","к","ко","об","обо",
    # Ответные и бытовые слова (дают ложные совпадения)
    "нет","да","ну","ок","окей","ладно","хорошо","понятно","именно",
    "точно","конечно","спасибо","пожалуйста","надо","надобно","всё","все",
    "привет","здравствуйте","добрый","день","утро","вечер","пока",
    "тут","здесь","там","ага","угу","ясно","ладно","давай","давайте",
    # Глаголы заказа и указания
    "поставь","ставь","оформи","запиши","бери","возьму","возьмём","хватит",
    "скинь","покажи","скажи","дай","дайте","подбери","выбери","найди",
    # Местоимения-указатели (вызывают ложные совпадения)
    "одно","одна","одну","один","оба","обе","несколько","пару","любой",
    "которая","который","которое","которые","такой","такая","такие",
    # Прилагательные-сравнения
    "очень","более","менее","дешевле","дороже","лучше","хуже","больше",
    "меньше","также","тоже","ещё","еще","просто","только","уже","ещё",
    # Цены и количество
    "стоит","цена","цены","рублей","рубль","тысяч","тысячи","штук","шт",
    # Единицы измерения (дают ложные совпадения в описаниях товаров)
    "квт","вт","квтч","гвт","литр","литра","литров","литры","кг","грамм",
    "метр","метра","метров","мм","см","дюйм","дюймов","герц","ггц","мгц",
}

# ─── Synonym map for colloquial words ─────────────────────────────────────────
...
_SYNONYMS = {
    ...
    "посудомойки": "посудомоечная",
    "морозилка":   "морозильная",
    "морозилки":   "морозильная",
    "варочная":    "варочная",
    "духовка":     "духовой",
    "духовки":     "духовой",
}
...
_SYSTEM_PROMPT = """\
Ты — {name}, менеджер «{company}». Пишешь как живой человек — кратко, по-деловому, без официоза. Не раскрывай, что ты ИИ.

ГЛАВНОЕ ПРАВИЛО
Цены, модели, наличие — только из блока [КАТАЛОГ] текущего сообщения. Никогда ничего не придумывай. Если товара нет в [КАТАЛОГ] — скажи «уточню у поставщика».

КАК ОТВЕЧАТЬ
Приветствие → поздоровайся и спроси что нужно. Одна фраза.
Запрос без деталей → задай один уточняющий вопрос (площадь, бюджет, размер).
Есть товар в [КАТАЛОГ] → покажи до 3 вариантов, каждый на новой строке «Название — цена». Потом один короткий вопрос.
Нет товара в [КАТАЛОГ] → скажи коротко «Нет» или «Уточню у поставщика», предложи аналог если он есть.
Клиент отвечает на вопрос или спрашивает что-то конкретное → отвечай на это, не повторяй список заново.
«Беру», «оформи», «заказываю», «возьму» → спроси имя и телефон. Потом create_order, notify_supplier.

СТИЛЬ (реальный, из переписок)
Коротко: «Есть, 2 шт» / «Нет пока» / «Под заказ, 7-10 дней» / «Принято» / «Хорошо 🤝»
Если дорого: «На какой бюджет считать?»
Если торгуются: «Посмотрю что можно сделать» / «Уточню у поставщика»
Если нет точного — аналог: «Такого нет, есть похожее — [название] за [цена]»

ЗАПРЕЩЕНО
Markdown, нумерация, тире в начале строк. Начинать с «Конечно», «Отлично», «Рад помочь», «Хороший выбор». Перечислять категории. Больше одного вопроса за раз. «Например, вы ищете...» — никогда.

ПРИМЕРЫ

Клиент: Привет
Ты: Привет! Что подобрать?

Клиент: Добрый день, есть стиральная машина на 6 кг?
Ты: Здравствуйте 👋 Какой бюджет?

Клиент: тысяч до 25
Ты: Beko WSPE6H616W — 21 000₽
Homeline WMWI6110 — 22 500₽
Какая нравится?

Клиент: кондер нужен на 20 квадратов
Ты: На какой бюджет смотрим?

Клиент: тысяч 30
Ты: Midea MSAG2-09N8C2S — 28 590₽
Royal Clima RCI-AR22HN — 24 000₽
Ultima Comfort EXD-I09PN — 19 900₽
Что берём?

Клиент: есть бойлер на 50 литров?
Ты: Есть, 9 550₽. Брать?

Клиент: а на 80?
Ты: 80й тоже есть — 11 550₽

Клиент: дорого
Ты: На какой бюджет считать?

Клиент: такой модели нет?
Ты: Нет пока, уточню у поставщика когда будет

Клиент: возьму первый
Ты: Оформляю. Как вас зовут и телефон?\
"""
...
class SalesAgent:
    ...
    def _rag(self, text: str) -> str:
        ...
        # Extract meaningful tokens (apply synonyms, drop pure numbers and stop-words)
