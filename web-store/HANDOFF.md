...
- Начат шаг 2 по фасетам: добавлена Prisma-модель `ProductAttribute`, чистый extractor атрибутов из названий и команда `npm run sync:attributes` для backfill `source=name`. Извлекаются производительность, объем бака, диагональ ТВ, разрешение, Smart TV, RAM, тип и объем накопителя. `spec`-фильтры для доступных атрибутов уже строят Prisma `where` через `attributes.some`, сохраняя fallback на название/поставщика.
